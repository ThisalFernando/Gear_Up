    ...
    <section>
        <div class="main_container">
            ...
            <div class="container">
                ...
                <form method = "POST" action = "orderUpdate.php" name="form1" id="detailsForm">
                    <input type="hidden" name="oid" value="<?= $row['oid'] ?>">
                    <div>
                        <label for = "fnameLbl" class="lblFname">Name of buyer: </label><br>
                        <input type="text" name="fName" id="fName" placeholder="Enter Full Name" value="<?= $row['Name'] ?>"/>
                    </div><br>

                    <div>
                        <label for = "addLbl" class="lbladd">Address: </label><br>
                        <input type="text" name="add" id="add" placeholder="Enter address" value="<?= $row['Address'] ?>"/>
                    </div><br>

                    <div>
                        <label for = "payLbl" class="payGen">Payment Method: </label><br>
                        <input type="text" name="pay" id="pay" placeholder="Enter payment method (Cash/Card)" value="<?= $row['PaymentMethod'] ?>"/>
                    </div><br>

                    <div>
                        <label for = "phoneLbl" class="lblPhone">Phone Number: </label><br>
                        <input type="text" name="tel" id="tel" placeholder="Enter phone number" maxLength = 10 value="<?= $row['Phone'] ?>"/>
                    </div><br>
            
                    <div>
                        <button type="Submit" value="Update Order" class="signupBtn">UPDATE ORDER</button><br>
                    </div>
